...
def train_one_epoch(model: nn.Module, 
    ...
    for i, (token_ids, lengths, images, y, y_acc) in enumerate(loader):
        output = model(token_ids, lengths, images)
        loss = loss_fn(output, y)
        loss /= grad_accum_size
        loss.backward()
        pred = max_to_one_hot(output)
...
def trainjob(model: nn.Module, 
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_acc = train_one_epoch(model, train_loader, loss_fn, optimizer, grad_accum_size, acc_fn)
        print(f"\tTrain Loss: {train_loss} | Train Accuracy: {train_acc*100}%")
        train_losses.append(train_loss)
        train_accs.append(train_acc)

        val_loss, val_acc = val_step(model, val_loader, loss_fn, acc_fn)
